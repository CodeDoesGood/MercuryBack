...
describe('Volunteer Component', () => {
  ...
  describe('#compareAuthenticatingPassword', () => {
    it('Should return true if the stored salt and hash match the passed password', () => {
      const volunteer = new Volunteer();
      volunteer.salt =
        '5HomjhDxeSNT4H9JmD3iW7h0iUjLyDG7XnKbvQK+QYxN914GExndCEKI5azGaSNSZKcgPrjyxvO04GO/QN6FtRpaTYbOF3DAVU9U6i' +
        '+1VfEMWcvOgb7sUmOb/OI48WRiPgcmatDmPMz89Ih0IspYq/Po+/UOnOMCEmDkfrfhlK8=';

      volunteer.password =
        '58b29f79f1861119815a8374995530d84f05fd3532ee38e55fb796d35356911a953bb45a49be8803ceffb12f49e6e1ab1bd1' +
      ...
      assert.equal(
        ...
        true,
        'Compare authentication password failed to return true with valid salt and password',
      );
    });
    ...
    it('Should return false if the stored salt and hash matches are wrong', () => {
      ...
      assert.equal(
        utils.compareAuthenticatingPassword('password', volunteer.salt, volunteer.password),
        false,
        'Compare authentication password passed when the salt was invalid',
      );
    });
    ...
    it('Should return false if no password is passed', () => {
      ...
      assert.equal(
        ...
        false,
        'Compare authentication password passed when the password was not given',
      );
    });
    ...
    it('Should return false if there is no passed in the class', () => {
      ...
      volunteer.salt =
        '5HomjhDxeSNT4H9JmD3iW7h0iUjLyDG7XnKbvQK+QYxN914GExndCEKI5azGaSNSZKcgPrjyxvO04GO/QN6FtRpaTYbOF3D' +
        'AVU9U6i+1VfEMWcvOgb7sUmOb/OI48WRiPgcmatDmPMz89Ih0IspYq/Po+/UOnOMCEmDkfrfhlK8=';

      assert.equal(
        utils.compareAuthenticatingPassword(undefined, volunteer.salt, volunteer.password),
        false,
        'Compare authentication password passed when the password was not even set',
      );
    });
  });
  ...
  if (_.isNil(process.env.TRAVIS)) {
    describe('#getProfile', () => {
      it('Should return all profile selections', () => {
        ...
        return volunteer.existsByUsername().then(
          () => {
      ...
      it('Should return true from the database if the user is able able to access the administration panel', () => {
        ...
        return administrator
          ...
          .then(() =>
            administrator
              .knex('volunteer')
              .update('admin_portal_access', true)
              .where('volunteer_id', administrator.userId),
          )
          .then(() => administrator.canAccessAdminPortal())
          .then((canAccessAdminPortal: boolean) => {
            ...
            return administrator
              .knex('volunteer')
              .update('admin_portal_access', false)
              .where('volunteer_id', administrator.userId);
          })
          .then(
            () => assert(true),
      ...
      it('Should create the user in the database and create a verification code', () => {
        ...
        return volunteer
          ...
          .then(() =>
            volunteer
              .knex('volunteer')
              .where('volunteer_id', volunteer.userId)
              .del(),
          )
          .then(
            () => assert(true),
          ...
          .then(
            (notifications: IAnnouncement[]) => {
              assert.equal(
                ...
                true,
                `Should return null if there is no notification, notifications=${notifications}`,
              );
              volunteer
                .knex('volunteer_announcement')
                .update('read', false)
    ...
    describe('#dismissNotification', () => {
      ...
      it('Should reject if the volunteer id does not exist', () => {
        ...
        return volunteer.dismissNotification(1).then(
          () => {
      ...
      it('should not mark it as read if its not the correct voluteer_id', () => {
        ...
        return volunteer
          ...
          .then(
            (newNotification: IAnnouncement[]) => {
              assert.equal(
                totalNotifications,
                newNotification.length,
                'new Notifications should not change when the userId is not correct',
              );
            },
            (error: Error) => {
              throw error;
      ...
      it('Should reject if the announcement id is not passed', () => {
        ...
        return volunteer.dismissNotification(null).then(
          () => {
