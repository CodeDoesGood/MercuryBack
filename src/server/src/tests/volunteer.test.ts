...
import Volunteer, { IAnnouncement, IPasswordResetCode, IVerificationCode } from '../components/Volunteer';
...
describe('Volunteer Component', () => {
  ...
  describe('#compareAuthenticatingPassword', () => {
    it('Should return true if the stored salt and hash match the passed password', () => {
      ...
      assert.equal(
        volunteer.compareAuthenticatingPassword('password'),
    ...
    it('Should return false if no password is passed', () => {
      ...
      assert.equal(
        volunteer.compareAuthenticatingPassword(undefined),
  ...
  if (_.isNil(process.env.TRAVIS)) {
      ...
      it('Should reject if the volunteer doesn\'t exists', () => {
        ...
        return volunteer.exists()
          .then(() => {
            ...
            assert.equal(
              error.message,
              'Volunteer does not exist by type=volunteerId', error.message);
          });
      });

      it('Should reject if the volunteerId is not passed', () => {
        const volunteer = new Volunteer();
        ...
        return volunteer.exists()
          .then(() => {
            ...
            assert.equal(
              error.message,
              'Type must be defined or valid, type=undefined', error.message);
          });
      });

      it('Should resolve when passing something other than the volunteerId', () => {
        const volunteer = new Volunteer(null, 'user1');
    ...
    describe('#removeVerificationCode', () => {
      it('Should reject if the volunteerId is not valid', () => {
        const volunteer = new Volunteer();

        return volunteer.removeVerificationCode()
          .then(() => {
            throw new Error('Shouldn\'t resolve when the volunteerId is invalid');
          },    (error: Error) => {
            assert.equal(error.message, `volunteerId "${volunteer.volunteerId}" passed is not a valid number`, error.message);
          });
      });
      ...
      it('Should reject if no password reset code exists', () => {
        ...
        return volunteer.exists('username')
          ...
          .then(() => {
            ...
            assert.equal(error.message, `No password reset code exists for user ${volunteer.volunteerId}`, error.message);
          });
      });
    ...
    describe('#create', () => {
      ...
      it('Should reject if the volunteer name is undefined or null', () => {
        ...
        return volunteer.create(undefined, undefined)
          .then(() => {
      ...
      it('Should reject if no verification code exists', () => {
        ...
        return volunteer.exists('username')
          ...
          .then(() => {
            ...
            assert.equal(error.message, `No verification code exists for user ${volunteer.volunteerId}`, error.message);
          });
      });
    ...
    describe('#dismissNotification', () => {
      ...
      it('should not mark it as read if its not the correct voluteer_id', () => {
        ...
        let oldVolunteerId: number = 0;

        return volunteer.exists('username')
          .then(() => volunteer.getActiveNotifications())
          .then((notifications: IAnnouncement[]) => {
            totalNotifications = notifications.length;
            oldVolunteerId = volunteer.volunteerId;
            volunteer.volunteerId = 99;
            return volunteer.dismissNotification(1);
          })
          .then(() => {
            volunteer.volunteerId = oldVolunteerId;
